...
def _build_dataclass_server(
    *, model_name: str, update_weights: bool, num_engines: int = 2, num_groups: int = 1
) -> RolloutServer:
    groups = [make_dataclass_group(num_engines=num_engines) for _ in range(num_groups)]
    for g in groups:
        g.update_weights = update_weights
    return RolloutServer(server_groups=groups, model_name=model_name, update_weights=update_weights)


def _build_fake_rollout_manager(servers: dict[str, RolloutServer]):
    """Bypass __init__ (which calls start_rollout_servers)."""
...
class TestRolloutManagerClearHasNewEnginesFlag:
    def test_clears_flag_on_updatable_server(self):
        srv = _build_dataclass_server(model_name="actor", update_weights=True, num_engines=2, num_groups=2)
    ...
    def test_no_op_when_no_updatable_server(self):
        ...
        RolloutManager.__ray_actor_class__.clear_updatable_has_new_engines(fake)
        # ref must NOT be cleared (it's not updatable)
