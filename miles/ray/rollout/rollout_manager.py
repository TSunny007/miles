...
class RolloutManager:
    ...
    async def generate(self, rollout_id):
        ...
        # When delay_split is set (auto-enabled for indep-DP / FT), hand the full
        # un-split data to the train side so each cell splits per its own (dynamic)
        # DP; otherwise pre-split by DP here. Both consumers (RayActorGroup and the
        # indep-DP RayTrainGroup) read ``data_ref`` off this dict.
